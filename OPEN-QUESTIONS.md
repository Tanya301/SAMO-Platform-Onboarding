...
- [ ] **Pick a harness so the bot never drops the loop** (issue → PR → review → CI). Nik suggests evaluating [flueframework](https://flueframework.com) (durable execution + sandboxes + GitHub/MCP; fits on paper, unverified). Candidate, not a decision — eval questions in [bring-up](docs/bot-bringup-and-harness.md#harness-evaluate-flueframework).
- [ ] **Make preview links genuinely private?** Today previews are unlisted but publicly reachable — the hostname has no access gate (only the client app's own login protects data). Decide whether previews should be access-controlled (token / basic-auth / Cloudflare Access) so we can honestly call them private, or whether unlisted-public is acceptable for MVP.
